Scaling [_mysolution_--c81a] With Kubernetes
===========================
...
Based on your TML solution [_mysolution_--c81a] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`_mysolution_--c81a.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-_mysolution_--c81a.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingress_mysolution_--c81a.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f _mysolution_--c81a.yml

_mysolution_--c81a.yml
------------------------

.. important::
   Copy and Paste this YAML file: _mysolution_--c81a.yml - and save it locally.

   These YML files can also be found on GitHub at: https://github.com/NataliKobrii/fraud-detection-model/tree/main/tml-airflow/dags/tml-solutions/_mysolution_/ymls/_mysolution_--c81a
...
.. code-block:: YAML
   ...
   ################# _mysolution_--c81a.yml
     ...
     kind: Deployment
     metadata:
       name: _mysolution_--c81a
     spec:
       selector:
         matchLabels:
           app: _mysolution_--c81a
       ...
       template:
         ...
         spec:
           containers:
           - name: _mysolution_--c81a
             image: nataliiakobrii/_mysolution_--c81a-amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: PROJECTNAME
               value: '_mysolution_'               
             - name: SOLUTIONNAME
               value: '_mysolution_--c81a'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_ml_restapi_dag'
             - name: GITUSERNAME
               value: 'NataliKobrii'
             - name: GITREPOURL
               value: 'https://github.com/arimlabs/fraud-detection-model'
             ...
             - name: step4jsoncriteria # STEP 4
               value: 'uid=email,filter:allrecords~subtopics=total_price~values=total_price~identifiers=id~datetime=created_at~msgid=id~latlong=:'                           
             ...
             - name: step5processlogic # STEP 5 processlogic field can be adjusted here.  
               value: 'classification_name=fraud_prob:total_price_preprocessed_AnomProb=55,n'                                 
             - name: step5independentvariables # STEP 5 independent variables can be adjusted here.  
               value: 'total_price_preprocessed_AnomProb'                                                               
   ...
   ---
     ...
     kind: Service
     metadata:
       name: _mysolution_--c81a-visualization-service
       labels:
         app: _mysolution_--c81a-visualization-service
     spec:
       type: ClusterIP
       ports:
   ...
   ---
     ...
     kind: Service
     metadata:
       name: _mysolution_--c81a-external-service
       labels:
         app: _mysolution_--c81a-external-service
     spec:
       type: ClusterIP
       ports:
       - port: 80 # Ingress port, if using port 443 will need to setup TLS certs
...
.. code-block::
   ...
   kubectl port-forward deployment/_mysolution_--c81a 6060:6060
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-_mysolution_--c81a.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-_mysolution_--c81a.yml
-------------
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-_mysolution_--c81a.yml
